...
<div class="page-header">
    <div>
        <h4 class="page-title">Plan Builder</h4>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item active">Plan Builder</li>
            </ol>
        </nav>
    </div>
</div>

<p class="text-muted mb-3">Pick a thematic area to build — each one is its own workspace.</p>
...
<div class="plan-builder-grid">
    ...
    <a href="{{ route('plan-builder.show', $thematicArea) }}" class="plan-pick-card">
        <div class="plan-pick-project">{{ $thematicArea->project?->name ?? 'No project' }}</div>
        ...
        <div class="plan-pick-meta">
            ...
            <span>{{ $thematicArea->indicator_count }} indicator{{ $thematicArea->indicator_count === 1 ? '' : 's' }}</span>
        </div>
    </a>
    @endforeach

    @can('thematic-area.create')
    <button type="button" class="plan-new-card" data-quick-add="tpl-add-thematic-area" data-quick-add-title="New Thematic Area">
        <i class="mdi mdi-plus"></i> New thematic area
    </button>
    @endcan
    ...
    <div class="tbl-empty" style="grid-column: 1 / -1;">
        ...
        <p>No thematic areas assigned to you yet.</p>
    </div>
    @endif
</div>
...
<template id="tpl-add-thematic-area">
    <form method="POST" action="{{ route('thematic-areas.store') }}">
        ...
        <div class="d-flex form-actions">
            <button type="submit" class="btn btn-dark"><i class="mdi mdi-content-save-outline"></i> Create</button>
