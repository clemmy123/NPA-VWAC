...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <small class="text-muted">{{ __('Link this account to a reporting organization (e.g. a bank) so it can be scoped to specific indicators.') }}</small>
    </div>
    ...
    <div id="approval-level-field" class="col-md-4 mb-3 {{ $currentRole === 'Data Approver' ? '' : 'd-none' }}">
        ...
        <select name="approval_location_level" id="approval_location_level" class="form-control @error('approval_location_level') is-invalid @enderror">
            ...
            <option value="{{ $level }}" @selected(old('approval_location_level', $approvalAssignment?->location_level ?? null) === $level)>{{ __(ucfirst($level)) }}</option>
            @endforeach
        </select>
