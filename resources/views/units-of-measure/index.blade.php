...
<div class="page-header">
    <div>
        <h4 class="page-title">Units of Measure</h4>
    ...
    <a href="{{ route('units-of-measure.create') }}" class="btn btn-dark btn-sm"><i class="mdi mdi-plus"></i> New Unit</a>
</div>

<div class="table-card d-none d-md-block">
    <table class="table mb-0">
        <thead>
            <tr><th>#</th><th>Code</th><th>Name</th><th>Symbol</th><th>Status</th><th>Actions</th></tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td><span class="s-badge {{ $unitOfMeasure->is_active ? 's-active' : 's-inactive' }}">{{ $unitOfMeasure->is_active ? 'Active' : 'Inactive' }}</span></td>
                <td>
                    <a href="{{ route('units-of-measure.edit', $unitOfMeasure) }}" class="btn-icon" title="Edit"><i class="mdi mdi-pencil-outline"></i></a>
                    <form action="{{ route('units-of-measure.destroy', $unitOfMeasure) }}" method="POST" class="d-inline"
                          onsubmit="return confirm('Delete this unit of measure?');">
                        @csrf
                        @method('DELETE')
...
<div class="d-md-none">
    ...
    <div class="mob-card">
        <div class="mob-card-top">
            ...
            <span class="s-badge {{ $unitOfMeasure->is_active ? 's-active' : 's-inactive' }}">{{ $unitOfMeasure->is_active ? 'Active' : 'Inactive' }}</span>
        </div>
