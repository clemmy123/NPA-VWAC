...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        <label for="measurement_type_id" class="form-label">{{ __('Measurement Type') }}</label>
    ...
    <div class="col-md-6 mb-3">
        <label for="unit_of_measure_id" class="form-label">{{ __('Unit of Measure') }}</label>
    ...
    <div class="col-md-3 mb-3">
        <label for="collection_mode" class="form-label">{{ __('Collection Mode') }}</label>
        <select name="collection_mode" id="collection_mode" class="form-control">
            @foreach ($collectionModes as $option)
            <option value="{{ $option }}" @selected(old('collection_mode', $indicator?->collection_mode ?? 'progressive') === $option)>{{ __(ucfirst($option)) }}</option>
            @endforeach
        </select>
    </div>

    <div class="col-md-3 mb-3">
        <label for="aggregation_method" class="form-label">{{ __('Aggregation Method') }}</label>
        <select name="aggregation_method" id="aggregation_method" class="form-control">
            @foreach ($aggregationMethods as $option)
            <option value="{{ $option }}" @selected(old('aggregation_method', $indicator?->aggregation_method ?? 'sum') === $option)>{{ __(ucfirst($option)) }}</option>
            @endforeach
        </select>
    </div>

    <div class="col-md-3 mb-3">
        <label for="reporting_frequency" class="form-label">{{ __('Reporting Frequency') }}</label>
        <select name="reporting_frequency" id="reporting_frequency" class="form-control">
            @foreach ($reportingFrequencies as $option)
            <option value="{{ $option }}" @selected(old('reporting_frequency', $indicator?->reporting_frequency ?? 'quarterly') === $option)>{{ __(ucfirst($option)) }}</option>
            @endforeach
        </select>
    </div>

    <div class="col-md-3 mb-3">
        <label for="collection_scope" class="form-label">{{ __('Collection Scope') }}</label>
        <select name="collection_scope" id="collection_scope" class="form-control">
            @foreach ($collectionScopes as $option)
            <option value="{{ $option }}" @selected(old('collection_scope', $indicator?->collection_scope ?? 'national') === $option)>{{ __(ucfirst($option)) }}</option>
            @endforeach
        </select>
    </div>

    <div class="col-md-6 mb-3">
        <label for="reporting_location_level" class="form-label">{{ __('Reporting Location Level') }}</label>
        <select name="reporting_location_level" id="reporting_location_level" class="form-control">
            <option value="">—</option>
            @foreach ($locationLevels as $option)
            <option value="{{ $option }}" @selected(old('reporting_location_level', $indicator?->reporting_location_level) === $option)>{{ __(ucfirst(str_replace('_', '/', $option))) }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
            ...
            <option value="{{ $option }}" @selected(old('status', $indicator?->status ?? 'draft') === $option)>{{ __(ucfirst($option)) }}</option>
            @endforeach
        </select>
    ...
    <div class="col-12 mb-3">
        <div class="check-row">
            <div class="form-check">
                ...
                <label for="requires_location" class="form-check-label">{{ __('Requires Location') }}</label>
            </div>
            <div class="form-check">
                ...
                <label for="requires_activity" class="form-check-label">{{ __('Requires Activity') }}</label>
            </div>
            <div class="form-check">
                ...
                <label for="has_budget_implication" class="form-check-label">{{ __('Has Budget Implication') }}</label>
            </div>
            <div class="form-check">
                ...
                <label for="requires_evidence" class="form-check-label">{{ __('Requires Evidence') }}</label>
            </div>
            <div class="form-check">
                ...
                <label for="requires_hierarchical_approval" class="form-check-label">{{ __('Requires Hierarchical Approval') }}</label>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function rebuildThematics(selectedId) {
        ...
        placeholder.textContent = projectId
            ? @json(__('Select a thematic area…'))
            : @json(__('Select a project first…'));
