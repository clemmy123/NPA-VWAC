...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="period_type" id="period_type" class="form-control @error('period_type') is-invalid @enderror" required>
            ...
            <option value="{{ $type }}" @selected(old('period_type', $reportingPeriod?->period_type ?? 'quarter') === $type)>{{ __(ucfirst(str_replace('_', ' ', $type))) }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-4 mb-3">
        <label for="sequence" class="form-label">{{ __('Sequence') }}</label>
