...
class TamisemiLocationSeeder extends Seeder
{
        ...
        try {
            foreach ($file as $row) {
                if (! is_array($row) || $row === [null]) {
                    ...
                    $headers = array_map(fn ($v) => $this->clean($v), $row);
                ...
                $w = $this->upsert('ward', 'ward_id', $this->clean($record['ward_code'] ?? null), $this->clean($record['ward_name'] ?? null), ['division_id' => $dv->division_id ?? null, 'township_id' => $t->township_id ?? null]);
            ...
            DB::commit();
            $this->command?->info('TAMISEMI locations seeded successfully.');
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    private function upsert(string $table, string $pk, ?string $code, ?string $name, array $parents = []): ?object
    {
        ...
        $row = $code ? (clone $q)->where('code', $code)->first() : null;
        if (! $row && $name) {
            $by = (clone $q)->where('name', $name);
            foreach ($parents as $col => $val) {
                $by->where($col, $val);
            } $row = $by->first();
        }
        $payload = array_merge(['name' => $name, 'updated_at' => now()], $parents);
        if ($code) {
            $payload['code'] = $code;
        } $payload = array_filter($payload, fn ($v) => $v !== null);
        if ($row) {
            DB::table($table)->where($pk, $row->{$pk})->update($payload);

            return DB::table($table)->where($pk, $row->{$pk})->first();
        }
        if (! $name) {
            return null;
        } $payload['created_at'] = now();
        $id = DB::table($table)->insertGetId($payload, $pk);
    ...
    private function clean($value): ?string
    {
        ...
        if ($value === '' || strtoupper($value) === 'NULL') {
            return null;
        } $value = mb_convert_encoding($value,'UTF-8','UTF-8, Windows-1252, ISO-8859-1, ASCII');
